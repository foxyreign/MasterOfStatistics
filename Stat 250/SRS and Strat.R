...
pop.mean_ne + pop.mean_nc + pop.mean_s + pop.mean_w

# Compute for strata variances
strat_ne.var <- (1-nrow(agstrat_ne)/northeast) * (sd(agstrat_ne$ACRES92)/nrow(agstrat_ne))
strat_nc.var <- (1-nrow(agstrat_nc)/northcentral) * (sd(agstrat_nc$ACRES92)/nrow(agstrat_nc))
strat_s.var <- (1-nrow(agstrat_s)/south) * (sd(agstrat_s$ACRES92)/nrow(agstrat_s))
strat_w.var <- (1-nrow(agstrat_w)/west) * (sd(agstrat_w$ACRES92)/nrow(agstrat_w))
